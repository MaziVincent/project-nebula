	...
	return (
		<div className="flex overflow-y-auto max-md:pt-10">
			<div className="flex-1 bg-gray-50 px-4 py-4">
				...
				<h2 className="text-2xl font-bold mb-4 pt-2">
						...
						{data?.length > 0 ? (
							data?.map((props) => (
								<div
									...
									/>
									<h4 className="text-gray-500 text-xl uppercase font-bold mb-2">
										{props.title}
									</h4>
									<h3 className="text-xl alt-font font-bold mb-3">
										&#8358;
										{parseFloat(props.price?.$numberDecimal).toLocaleString(
											"en-US"
										)}{" "}
									...
									<button
										onClick={() => navigate(`/dashboard/details/${props._id}`)}
										className="mt-4 w-full bg-emerald-500 text-white px-4 py-1 rounded-lg">
										View
									</button>
								</div>
							))
