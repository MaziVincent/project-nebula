		...
		<nav className="bg-green-50 border-b border-gray-200 px-4 py-2.5 fixed left-0 right-0 top-0 z-50">
			...
			<div className="flex flex-wrap justify-between items-center">
				<div className="flex justify-start items-center gap-16">
					...
					<Link to="/agent" className="flex items-center justify-between mr-4">
				...
				<div className="flex justify-between items-center gap-2">
					<Link
						...
						className="h-14 w-14 rounded-full self-end relative"
						aria-label="View profile">
						<img
							src={auth?.user?.profile ? auth.user.profile : Profile}
							alt={`${auth?.user?.firstname || "User"} profile picture`}
							className="w-full h-full rounded-full object-cover relative"
						/>
						<span
							className="w-3.5 h-3.5 rounded-full bg-green-600 border-2 border-white block absolute top-9 right-0"
							aria-label="Online status indicator"></span>
					</Link>
				</div>
			</div>
		</nav>
