...
const OwnerOverview = () => {
	...
	const handleOpenHouse = useCallback(() => setOpenHouseModal(true), []);
	const handleCloseHouseModal = useCallback(() => setOpenHouseModal(false), []);

	const [openLandModal, setOpenLandModal] = useState(false);
	const handleOpenLand = useCallback(() => setOpenLandModal(true), []);
	const handleCloseLandModal = useCallback(() => setOpenLandModal(false), []);

	const [openShopModal, setOpenShopModal] = useState(false);
	const handleOpenShop = useCallback(() => setOpenShopModal(true), []);
	const handleCloseShopModal = useCallback(() => setOpenShopModal(false), []);

	//for update property modal
	const [openUpdate, setOpenUpdate] = useState(false);
	const [selectedProperty, setSelectedProperty] = useState(null);
	const handleOpenUpdate = useCallback((property) => {
	...
	const handleOpenStatus = useCallback(() => setOpenStatus(true), []);
	const handleCloseStatus = useCallback(() => setOpenStatus(false), []);
	...
	const getProperties = async () => {
		const response = await fetch(
			`${url}/${ownerId}?page=${page}&limit=10`,
			auth.accessToken
		);
	...
	return (
		<div className=" h-dvh">
			...
			{isSuccess && (
				<div>
					<h1 className=" text-2xl font-semibold mt-4 text-green-900 text-center">
						{" "}
						Overview
					</h1>
					<div>
						<div className=" flex justify-between items-center mb-4">
							<h2 className=" text-3xl font-normal text-start text-green-900 mb-0">
								Properties
							</h2>
							<div className=" relative">
								<button
									onClick={handlePropertyType}
									className=" bg-green-600 rounded-lg text-xl font-semibold text-emerald-50 px-2 py-2 inline-flex items-center">
									<span>
										<Add />
									</span>
									Add New Property
								...
								{propertyType && (
									<div className=" absolute bg-green-50 w-full px-2 rounded-lg py-2 z-50 shadow-lg">
										<h4 className=" text-lg mb-1 text-center text-green-700 tracking-widest">
			...
			{isError && (
				<div className="text-red-500 text-3xl text-center m-52">
					{error.message}
				</div>
			)}
			<div>
				<UpdatePropertiesModal
					...
					url={`${baseURL}${selectedPropertyType}`}
				/>
				<CreateNewApartmentModal open={openModal} handleClose={handleClose} />
				<CreateNewHouseModal
					open={openHouseModal}
					handleCloseHouseModal={handleCloseHouseModal}
				/>
				<CreateNewLandModal
					open={openLandModal}
					handleCloseLandModal={handleCloseLandModal}
				/>
				<CreateNewShopModal
					open={openShopModal}
					handleCloseShopModal={handleCloseShopModal}
				/>
				<PropertiesStatusModal
