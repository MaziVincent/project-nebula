...
const UploadHouseDocModal = ({
	...
	const { auth } = useAuth();
	const url = `${baseURL}house/docupload`;
	const { id } = useParams();
	...
	const uploadImage = async (data) => {
		...
		try {
			const response = await axios.put(`${url}/${houseId}`, formData, {
				headers: {
							...
							<div className="">
								<form
									...
									<div className=" mt-4 flex justify-end gap-2">
										...
										<button
											type="submit"
											className="bg-blue-600 px-6 py-1 rounded-lg text-white inline-flex items-center justify-center">
											{isLoading ? (
...
export default React.memo(UploadHouseDocModal);
