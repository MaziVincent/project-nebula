...
const ShopDetails = () => {
  ...
  return (
        ...
        <div>
          <div className=''>
            <div className=' leading-3 mb-4'>
              <h1 className='text-xl inline-flex gap-2 items-center mb-0 leading-none'>Title: <span className='text-lg font-semi-bold text-gray-800 uppercase'>{shop.title}</span></h1>
              <p className='m-0'>Owner: <span className=' text-gray-700 text-sm tracking-wider underline'>{shop?.owner?.firstname} {shop?.owner?.lastname}</span></p>
             </div>
            <div className=' grid grid-cols-3 gap-2 shadow-md duration-200 delay-100 ease-in-out overflow-hidden'>
              {
                shop.imageUrls.map((imageUrl, index) => (
                  <img key={index} src={imageUrl} alt={shop.title} className=' w-full h-full duration-300 delay-300 ease-in-out  hover:scale-105' />
                ))
              }
            </div>
            ...
            <div className=' grid grid-cols-4 gap-y-8 max-sm:grid-cols-2 w-11/12'>
              ...
              <p className=' border-r-2 border-gray-500 text-gray-600 p-2 m-0 pl-0 flex flex-col items-center'>ShopCategory: <span>{shop.shopCategory}</span></p>
              <p className=' border-r-2 border-gray-500 text-gray-600 p-2 m-0 pl-0 flex flex-col items-center'>LeaseDuration: <span>{shop.leaseDuration}</span></p>
              <p className=' text-gray-600 p-2 m-0 pl-0 flex flex-col items-center'>SecurityDeposit: <span>{shop.securityDeposit}</span></p>
            </div>
            <div>
          ...
          <div className=' flex justify-start items-start gap-4 my-4 '>
            ...
            <button
              ...
              >
              <span className=' max-sm:hidden md:block'>Delete Shop 
              </span>
              <span className=' md:hidden max-sm:block'>
              <svg 
              xmlns="http://www.w3.org/2000/svg" 
      ...
      <DeletePropertyModal openDelete={openDelete} handleDeleteClose={handleDeleteClose} propertyId={propertyId} url={url} />
