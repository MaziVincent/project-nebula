...
function AgentDetails() {
	...
	return (
		<div className=" max-md:pt-24">
						...
						<div className="">
							...
							<div className="md:w-2/3 md:pl-8 mt-4 md:mt-0">
								<h2 className=" text-xl font-light">
									Name:{" "}
									<span className=" text-2xl font-semibold text-gray-600">{`${agent.firstname} ${agent.lastname}`}</span>
								</h2>
								...
								<p className="mb-4 font-medium text-lg text-gray-600">
									Verification Status:{" "}
									<span
										className={`${
											agent.verified ? "text-blue-600" : " text-red-600"
										}`}>
										{agent.verified ? "Verified" : "Not Verified"}
									</span>
								</p>
								<p className="mb-4 text-gray-600">
									Identity Type: <span>{agent.identityType}</span>
								</p>
								<p className="mb-4 text-gray-600">
									Identity Number: <span>{agent.identityNumber}</span>
								</p>
								<p className="mb-4 text-gray-600">
									Contact Address: <span>{agent.contactAddress}</span>
								</p>
								<p className="mb-4 text-gray-600">
									Office Address: <span>{agent.officeAddress}</span>
								</p>
								<p className="mb-4 text-gray-600">
									Agency Name: <span>{agent.agencyName}</span>
								</p>
								<p className="mb-4 text-gray-600">
									Properties: <span>{agent.properties.title}</span>
								</p>
			...
			/>
			<UnverifyAgentModal
				openUnverify={openUnverify}
				handleUnverifyClose={handleUnverifyClose}
				agentId={agentId}
			/>
			<UpdateAgentModal
